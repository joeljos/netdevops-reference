...
def generate_noshut_config(device, exclude):
    # Device -> device object
    # exclude -> [int1, int2]

    # Get the list of interfaces up
    interface_to_noshut = interfaces_down(device)

    # For each of them, generate the configuration to shut them down, and unshut them
    configuration = []
    for interface in interface_to_noshut:
...
def interfaces_down(device):
    if device.os == 'iosxe':
        output = device.parse('show interfaces')
    else:
        output = device.parse('show interface')

    return output.q.contains_key_value('oper_status', 'down').get_values('[0]')
...
if __name__ == '__main__':
    ...
    for device in args.devices:
        ...
        to_shut.append(generate_noshut_config(dev_obj, excludes.get(device, [])))

    for configuration in to_shut:
        device, config = configuration
        print("reverting - 'Shutting down interfaces of device '{d}''".format(d=device.name))
