...
curl -s --header "PRIVATE-TOKEN: $personal_access_token" -d "email=developer@devbox.cisco.com&password=devbox&username=developer&name=developer&skip_confirmation=true" "http://127.0.0.1/api/v4/users" 2>&1 >> gitlab_setup.log
success

echo "Updating git global config on devbox"
git config --global user.name "developer"
git config --global user.email developer@devbox.cisco.com

printf "Dont forget to register your GitLab Runner ... "